...
// Granularity (seconds per candle) for the current detectPatterns() run.
// Set at the top of detectPatterns(); base() uses it to stamp detectedAt at
// the last closed candle's CLOSE. Module-level is safe: detection is fully
// synchronous, so no interleaving between concurrent calls.
let currentGranularity = 3600;
...
function base(candles, startIndex, endIndex) {
  // Stamp detection at the last (most recent closed) candle's CLOSE, not its
  // open: the pattern is confirmed when that candle closes. Candle `time` is
  // the bucket open, so add one granularity to get the close.
  const last = candles[candles.length - 1];
  return {
    detectedAt: new Date((last.time + currentGranularity) * 1000).toISOString(),
    startIndex,
    endIndex,
  };
}
 ...
 */
export function detectPatterns(candles, granularity) {
  if (!Array.isArray(candles) || candles.length < 60) return [];
  currentGranularity = Number(granularity) || 3600;
