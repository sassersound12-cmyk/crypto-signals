 ...
 * other side of the level). Dedupe: each distinct break event
 * (symbol|type|level|candle time) emits at most one signal — tracked in
 * memory so repeated polling does not duplicate signals.
 *
...
/** In-memory dedupe: every distinct break event emits at most one signal. */
const emittedBreaks = new Set(); // `${symbol}|${type}|${levelKey}|${breakTime}`

function levelKey(level) {
  return Number(level.toPrecision(4)).toString();
}
 ...
 * Only the newest occurrence of each distinct break event is emitted, and
 * each event is emitted at most once across calls (in-memory dedupe set).
 */
function scanBreaks({
  symbol,
  asset,
  ...
  // Newest first so the latest break wins the dedupe for its level.
  for (let i = candles.length - 1; i >= 20; i--) {
    ...
    const key = `${symbol}|${type}|${levelKey(level)}|${c.time}`;
    if (emittedBreaks.has(key)) continue;
    emittedBreaks.add(key);
